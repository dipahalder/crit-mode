...
// Maren's full layout (Versed-like airy centered editorial). Extended for M12:
// the page-level `concept` reflows the hero treatment (image-first for the
// proof-led story) and the routine/essentials/testimonial order. Editable
// fields render from `view`; every string passes through clean() (guardrail 4).
...
export default function MarenLayout({ brand: b, view, register }: { brand: Brand; view: Page; register: RegisterTarget }) {
  ...
  const routineSection = (
    <div key="routine" style={{ padding: '40px 48px 46px', borderTop: '1px solid var(--line)' }}>
  ...
  const essentialsSection = (
    <div key="essentials" style={{ padding: '44px 48px 52px', borderTop: '1px solid var(--line)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 26 }}>
        {b.feats.map((f) => (
          <div key={f.name} style={{ textAlign: 'center' }}>
            <div style={{ height: 200, borderRadius: 14, background: 'var(--hero)', backgroundImage: 'repeating-linear-gradient(135deg,rgba(0,0,0,.04) 0 11px,transparent 11px 22px)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'ui-monospace, monospace', fontSize: 10.5, color: 'rgba(0,0,0,.35)', marginBottom: 16 }}>{clean(b.featImg)}</div>
  ...
  const testimonialSection = (
    <div key="testimonial" style={{ padding: '56px 48px', background: 'rgba(0,0,0,.022)', borderTop: '1px solid var(--line)', textAlign: 'center' }}>
  ...
  // M12: ritual-led leads with the routine; proof-led leads with the testimonial
  // (and an edge-to-edge image-first hero); essentials-led is the default.
  const body =
    concept === 'ritual-led'
      ? [routineSection, essentialsSection, testimonialSection]
      : concept === 'origin-led'
        ? [testimonialSection, essentialsSection, routineSection]
        : [essentialsSection, routineSection, testimonialSection]

  return (
    <>
      {/* Nav: links left, centered wordmark (anchors the concept pin), signup right */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px 48px' }}>
        ...
        <span ref={register('concept')} style={{ fontFamily: 'var(--display)', fontWeight: 600, fontSize: 23, letterSpacing: '3px', color: 'var(--ink)' }}>{clean(nameCaps)}</span>
